...
class AppResetService {
  Future<void> reset({
    ...
    final preservedUserIds = <String>{myId}
      ..addAll(_extractAuthProfileIds(authProfiles));
    if (loggedIn is String && loggedIn.trim().isNotEmpty) {
      preservedUserIds.add(loggedIn);
    }
    if (currentProfileId is String && currentProfileId.trim().isNotEmpty) {
      preservedUserIds.add(currentProfileId);
    }
    ...
    if (authProfiles != null) {
      await settings.put('auth_profiles_v2', authProfiles);
    }
    if (currentProfileId != null) {
      await settings.put('auth_current_profile_id', currentProfileId);
    }
    ...
    await _cleanStaleUsers(
      usersBox: usersBox,
      preservedUserIds: preservedUserIds,
      activeUserId: myId,
    ...
    for (var def in teamDefinitions) {
      ...
      List<Map<String, String>> membersForTask = [
      ...
      for (var u in tUsers) {
        ...
        if (!usersBox.containsKey(uid)) {
          await usersBox.put(
            uid,
            AppUser(id: uid, password: '1', name: u['name']!),
          );
        }
        memberIds.add(uid);
      ...
      final team = Team(
        id: teamId,
        name: tName,
        inviteCode: 'CODE_${random.nextInt(9999)}',
        memberIds: memberIds,
      ...
      for (int i = 0; i < 20; i++) {
        ...
        final createdAt = dueDate.subtract(
        ...
        final task = Task(
          ...
          projectId: random.nextDouble() > 0.15 ? project.id : null,
          createdAt: createdAt,
          dueDate: dueDate,
          updatedAt: isDone ? dueDate.add(const Duration(hours: 2)) : createdAt,
      ...
      final j1 = JournalEntry(
        ...
        userName: myName,
        title: '샘플 일지 ($tName)',
        content: '자동 생성된 데이터입니다.',
        date: now,
        photos: ['https://picsum.photos/seed/${random.nextInt(100)}/800/800'],
      );
      await Hive.box<JournalEntry>('journals').put(j1.id, j1);
    }
  }

  Set<String> _extractAuthProfileIds(dynamic rawProfiles) {
    final ids = <String>{};
    if (rawProfiles is! List) return ids;
  ...
  Future<void> _cleanStaleUsers({
    required Box<AppUser> usersBox,
    required Set<String> preservedUserIds,
    required String activeUserId,
    required String activeUserName,
  }) async {
    for (final key in usersBox.keys.toList()) {
      final id = key.toString();
      if (!preservedUserIds.contains(id)) {
        await usersBox.delete(key);
      }
    }

    final name = activeUserName.trim().isEmpty ? '사용자' : activeUserName.trim();
    final existing = usersBox.get(activeUserId);
    if (existing == null) {
      await usersBox.put(
        activeUserId,
        AppUser(id: activeUserId, password: '', name: name),
      );
      return;
    }

    existing.name = name;
    await usersBox.put(activeUserId, existing);
  }
}
