...
class Team {
  ...
  Team({required this.id, required this.name, required this.inviteCode, required this.memberIds});
  ...
  Map<String, dynamic> toJson() => {
    'id': id, 'name': name, 'inviteCode': inviteCode, 'memberIds': memberIds,
  };
}
...
class ChatMessage {
  ...
  ChatMessage({
    required this.id, required this.teamId, required this.senderId, 
  ...
  Map<String, dynamic> toJson() => {
    'id': id, 'teamId': teamId, 'senderId': senderId, 'senderName': senderName,
...
// --- 4. 사용자(AppUser) 모델 ---
class AppUser {
  final String id, password, name, role;
  final String? profileImage;

  AppUser({required this.id, required this.password, required this.name, required this.role, this.profileImage});

  factory AppUser.fromJson(Map<String, dynamic> json) => AppUser(
    id: json['id'], password: json['password'], name: json['name'], role: json['role'] ?? '팀원', profileImage: json['profileImage'],
  );

  Map<String, dynamic> toJson() => {'id': id, 'password': password, 'name': name, 'role': role, 'profileImage': profileImage};
}
